...
- [ ] **Weekly refiner for new users (first 6 weeks)** — New users need faster feedback loops since their initial profile is a rough estimate. For the first 6 weeks after account creation, run the refiner weekly (every Saturday, say) instead of on the biweekly schedule — but only if the user has at least one rating in the past 7 days. After 6 weeks, graduate them to the standard biweekly cadence automatically. **Implementation notes:** Add a `created_at` timestamp (ISO date string) to `taste_profile.json` at onboarding time (both `create_profile.py` and `process_pending.py`). The weekly cron pass checks `created_at`; if fewer than 42 days have elapsed and recent ratings exist, it runs the refiner with `--days 7`. A separate Saturday cron entry or a new `--new-users` mode in `run_all_users.py` handles the dispatch. No changes to the main biweekly cron needed — new users simply get an additional early pass.
