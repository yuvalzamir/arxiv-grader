...
- **Triage** (Claude Haiku, `prompts/triage.txt`): Receives lean profile (keywords, areas, authors) + all ~80 papers. Classifies high/medium/low. Hard caps: max 10 arXiv + 10 journal papers forwarded.
- **Scoring** (Claude Sonnet, `prompts/scoring_insights.txt`): Receives full profile + triage survivors. Outputs score 1–10 + an `insights` object (`claim`, `novelty`, `relevance`) per paper.

**Paper Insights (all users, by design):** Scoring always uses `prompts/scoring_insights.txt`; the insights object is rendered in the PDF as a three-row box below the author band, replacing the one-line justification and tags. Papers with truncated/missing abstracts are excluded from insights in Python (not just by prompt instruction). The legacy `"paper_insights"` flag in `taste_profile.json` is ignored.
