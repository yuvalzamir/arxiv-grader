...
| **ACS** | CrossRef JATS abstract (primary, since Silverchair migration); Europe PMC → OpenAlex fallback | ~75–100% full at fetch time |
...
| ~~ACS~~ (added 2026-07-20, moot since 2026-08-14) | `pubs.acs.org` | no longer fetched via RSS — ACS journals moved to CrossRef enumeration (see below); hostname left in `_CLOUDFLARE_HOSTS` in case ACS RSS URLs ever return |
...
**ACS RSS feeds killed by Silverchair migration (2026-07-24, discovered 2026-08-14):** ACS migrated all journals from Atypon to Silverchair on 2026-07-24; the `action/showFeed` RSS endpoints now return 404 (FlareSolverr solves the challenge fine — the resource is simply gone). Their migration page lists "Enablement of RSS feeds" as still forthcoming. Watermarks froze at 2026-07-22/23. **Resolution (2026-08-14):** all 7 ACS journals switched from RSS to **CrossRef enumeration** (`crossref_issn` in `fields.json`, same path as AER/AEJ) — immune to publisher platform churn, no bot bypass needed for the article list. `fetch_from_crossref()` extended to run the publisher scraper: `editorial_filter` (corrections/errata) + abstract fill via `scrape_article` when CrossRef has none. Bonus: post-migration ACS deposits JATS abstracts to CrossRef (~75–100% full in testing), so CrossRef is now also the primary ACS abstract source; Europe PMC/OpenAlex fill gaps. CrossRef freshness verified same-day (papers dated today appear in the API). Licensing improves too: article lists + abstracts now come from CrossRef's open metadata rather than a bot-protection bypass of pubs.acs.org. Old feed-URL watermark keys in `journal_watermarks.json` are stale-but-harmless; new `crossref:<issn>` keys start with the default 2-day lookback (no backfill flood; papers Jul 23–Aug 12 are lost — feeds were dead and CrossRef backfill would exceed the 50-row window anyway). **Confirmed in production 2026-08-17** (first cron run post-deploy): `Publisher 'acs': 2 paper(s) collected` — low count expected since the server test consumed the backlog.
